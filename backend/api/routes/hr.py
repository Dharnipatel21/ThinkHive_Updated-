...
@router.post("/members", status_code=201)
async def add_member(member: MemberCreate, context: TenantContext = Depends(require_permission("hr:manage")), svc: HRService = Depends(get_svc)) -> dict:
    return await svc.add_member(context, member)

@router.post("/bulk-upload", response_model=BulkUploadResult)
async def bulk_upload(file: UploadFile = File(...), context: TenantContext = Depends(require_permission("hr:manage")), svc: HRService = Depends(get_svc)) -> BulkUploadResult:
    return await svc.bulk_upload_csv(context, file)